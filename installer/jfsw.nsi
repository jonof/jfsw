!define PRODUCT_NAME "JFShadowWarrior"
!define PRODUCT_VERSION "20051009"
!define PRODUCT_PUBLISHER "JonoF"
!define PRODUCT_WEB_SITE "http://www.jonof.id.au/index.php?p=jfsw"
...
OutFile "jfsw-${PRODUCT_VERSION}-setup.exe"
...
Section "!Game (required)"
	...
	CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\JFShadowWarrior.lnk" "$INSTDIR\sw.exe"
	CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\JFShadowWarrior Setup.lnk" "$INSTDIR\sw.exe" "-setup"
...
Section -AdditionalIcons
	...
	CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\JFShadowWarrior Website.lnk" "$INSTDIR\${PRODUCT_NAME}.url"
...
Section -Post
	...
	WriteRegStr HKLM "${PRODUCT_DIR_REGKEY}" "" "$INSTDIR\sw.exe"
	WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "DisplayName" "$(^Name)"
	WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "UninstallString" "$INSTDIR\uninst.exe"
	WriteRegStr ${PRODUCT_UNINST_ROOT_KEY} "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\sw.exe"
...
Section Uninstall
	...
	Delete "$SMPROGRAMS\${PRODUCT_NAME}\JFShadowWarrior.lnk"
	Delete "$SMPROGRAMS\${PRODUCT_NAME}\JFShadowWarrior Setup.lnk"
	...
	Delete "$SMPROGRAMS\${PRODUCT_NAME}\JFShadowWarrior Website.lnk"
	Delete "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk"

	Delete "$INSTDIR\sw.grp"
	Delete "$INSTDIR\sw.cfg"
